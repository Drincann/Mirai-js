...
// 扩展接口
const { MessageChainGetable, BotConfigGetable, ForwardNodeGetable } = require('./interface');
...
class Bot extends BotConfigGetable {
     ...
     */
    async sendForward({ temp = false, friend, group, nodeList }) {
        // 检查对象状态
        if (!this.config) {
        ...
        if (temp) {
            ...
            // 而正常的好友和群的发送消息接口是分开的，所以在外面做了参数判断并抛出异常，格式相同
            return await _sendTempMessage({
                baseUrl, sessionKey, qq: friend, group, messageChain: message
            });
        } else {
            if (friend) {
                return await _sendFriendMessage({
                    baseUrl, sessionKey, target: friend, messageChain: message
                });
            } else if (group) {
                return await _sendGroupMessage({
                    baseUrl, sessionKey, target: group, messageChain: message
                });
            } else {
                throw { message: 'sendGroupMessage 缺少必要的 qq 或 group 参数' };
            }
